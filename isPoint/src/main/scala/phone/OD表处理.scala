...
object OD表处理 {
  def main(args: Array[String]): Unit = {
    ...
    val data = sc.textFile("D://项目//phone//经纬度文件//test_data.txt").map(i=>i.split(" ")).collect()
    val data_OD1 = sc.textFile("D://项目//phone//经纬度文件//test_OD.txt").map(i=>i.split(" ")).collect()
    val data_OD = sc.textFile("C://Users//pc//Desktop//草稿.txt").map(i=>i.split(" ")).collect()
    val data2 = sc.textFile("D://项目//phone//行政区//shp2.txt").map(i=>i.split(" "))
    data2.foreach(i=>{
      println(isInPolygon( "106.508" ,"29.54", i(1))+i(0))
    })
